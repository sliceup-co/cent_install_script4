#!/bin/bash
    echo -e "\e[96m Do not run this script with sudo. The script will use sudo if appropriate.\e[39m"
...
#Check to see if Java working on remote node.
	...
	echo -e "\e[96m Success! All devices running correct Java version. Continuing install of Prime node. \e[39m"
    sleep 2
...
###If data mount is not default e.g. $s=n
	            ...
	            if [ "$s" = "N" ] || [ "$s" = "n" ]; then
                    sudo mkdir -p $mount/.sliceup-data/
                    sudo mkdir -p $mount/.sliceup-data/kafka
                    sudo mkdir -p $mount/.sliceup-data/pgdata
...
#Lock this down and standardize install
    ...
    sudo sed -i "s/#listen_addresses.*/listen_addresses = '*'/" $dbdir/postgresql.conf
...
# take user entered IP addresses and create hba config
    ...
    sudo sed -i "s/# IPv4 local connections:/# IPv4 local connections:\n$line/" $dbdir/pg_hba.conf
    sudo sed -i 's/\(host  *all  *all  *127.0.0.1\/32  *\)ident/\1md5/' $dbdir/pg_hba.conf
    sudo sed -i 's/\(host  *all  *all  *::1\/128  *\)ident/\1md5/' $dbdir/pg_hba.conf
